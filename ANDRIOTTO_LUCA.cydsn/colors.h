/* ========================================
 *
 * Andriotto Luca, Assignment #2
 *
 * ========================================
*/

#include "project.h"
#include "RGBLedDriver.h"

// TIPI DI ONDA QUADRA
// 1. OFF
#define CMPMODE1R PWM_RED__B_PWM__GREATER_THAN_OR_EQUAL_TO          // UTILIZZO LE PRIME DUE MASCHERE PER EVITARE ACCENSIONE LED
#define CMPMODE1G PWM_GREEN__B_PWM__GREATER_THAN_OR_EQUAL_TO
// 2. INTERMITTENZA GREEN
#define CMPMODE2R PWM_RED__B_PWM__GREATER_THAN
#define CMPMODE2G PWM_GREEN__B_PWM__GREATER_THAN
// 3. INTERMITTENZA RED
#define CMPMODE3R PWM_RED__B_PWM__LESS_THAN
#define CMPMODE3G PWM_GREEN__B_PWM__GREATER_THAN
// 4. ALTERNANZA RED/GREEN
#define CMPMODE4R PWM_RED__B_PWM__GREATER_THAN
#define CMPMODE4G PWM_GREEN__B_PWM__LESS_THAN
// 5. ALTERNANZA RED/GREEN VELOCE
#define CMPMODE5R PWM_RED__B_PWM__LESS_THAN
#define CMPMODE5G PWM_GREEN__B_PWM__GREATER_THAN
// 6. OFF -- GREEN -- ORANGE -- CYCLE
#define CMPMODE6R PWM_RED__B_PWM__GREATER_THAN
#define CMPMODE6G PWM_GREEN__B_PWM__GREATER_THAN
// 7. RED -- ORANGE -- OFF -- GREEN -- CYCLE
#define CMPMODE7R PWM_RED__B_PWM__LESS_THAN
#define CMPMODE7G PWM_GREEN__B_PWM__GREATER_THAN

// N.B.: PER LE PROVE EFFETTUATE HO UTILIZZATO DUE RESISTORI SUI CATODI PER DIFFERENZIARE LE TONALITà.

/*

r           T_OFF (T_ON) VALUE DEL RED CHANNEL NEL TIPO "GREATER_THAN" ("LESS_THAN")
g           T_OFF (T_ON) VALUE DEL GREEN CHANNEL NEL TIPO "GREATER_THAN" ("LESS_THAN")
CMP_MOD_R   COMPARE MODE (RISING/FALLING EDGE INIZIALE) DEL RED CHANNEL
CMP_MOD_L   COMPARE MODE DEL GREEN CHANNEL
PERIOD_R    PERIODO ONDA QUADRA DEL RED CHANNEL
PERIOD_L    PERIODO ONDA QUADRA MODE DEL RED CHANNEL

*/
static Color color[] =  {
//                         r   g  CMP_MOD_R  CMP_MOD_G  PERIOD_R    PERIOD_G
                        {  0,  0, CMPMODE1R, CMPMODE1G, 255,        255},
                        {  0,128, CMPMODE2R, CMPMODE2G, 255,        255},
                        {128,  0, CMPMODE3R, CMPMODE3G, 255,        255},
                        { 64, 64, CMPMODE4R, CMPMODE4G, 127,        127},
                        { 32, 32, CMPMODE5R, CMPMODE5G,  63,         63},
                        { 64,128, CMPMODE6R, CMPMODE6G, 255,        255},
                        {128, 64, CMPMODE7R, CMPMODE7G, 255,        127}
                        };
